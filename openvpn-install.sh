...
async_new_client () {
		sudo mkdir -p /etc/openvpn/certificates
		# Generates the custom client.ovpn
        #client_ip=$(python3 /etc/openvpn/scripts/ip_process.py "$client")
        #if [ "$client_ip" != "1" ] || [ ! -z "$client_ip" ]; then{
            #echo $(python3 /etc/openvpn/scripts/ip_process.py "$client")
		...
		echo "</tls-crypt>"
} > /etc/openvpn/certificates/"$client".ovpn  #/etc/openvpn/ccd/"$client".ovpn
        #else
		#echo "You need to Increase the Pool Size"
        #    exit
        #fi
new_pool() {
    sudo mkdir -p /etc/openvpn/hub_clients

    client_pool=$(python3 '/etc/openvpn/scripts/assigns_pool_ip.py' "$VPN_HUB" "$range")

    if [ "$client_pool" != "1" ]; then
        {
            echo "$client_pool"
        } > /etc/openvpn/hub_clients/"$VPN_HUB"  #/etc/openvpn/hub_clients/"$VPN_HUB"
    else
        echo "The pool is Filled!!!"
        echo "Create another Your requirement!"
        exit
    fi
}
...
new_hub_client() {
    ...
    client_pool_ip=$(python3 '/etc/openvpn/scripts/assign_client_to_their_pool.py' "$HUB" "$client_name")
    if [ "$client_pool_ip" == "1" ] || [ "$client_pool_ip" == '' ]; then
        echo "All fields are filled in Your Hub:"
        exit
    elif  [ "$client_pool_ip" != "1" ] || [ "$client_pool_ip" != "2" ] ; then
        {
            ...
            sed -ne '/BEGIN OpenVPN Static key/,$ p' /etc/openvpn/server/tc.key
            echo "</tls-crypt>"
        } > /etc/openvpn/ccd/"$client_name".ovpn  # /etc/openvpn/ccd/"$client_name".ovpn
    # elif [ "$client_pool_ip" == "1" ] || [ -z "$client_pool_ip" ] ;then
    else
        echo "Change the Netmask for Giving an Ip"
        exit
    fi
}
...
verb 3" > /etc/openvpn/server/client-common.txt
	...
	echo "Finished!"
	echo
	echo "The client configuration is available in:" /etc/openvpn/certificates/"$client.ovpn"
