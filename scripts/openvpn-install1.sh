...
new_client () {
	...
	# Generates the custom client.ovpn
        ...
        if [ "$client_ip" != "1" ] && [ "$client_ip" != ' ' ]; then
        {
            ...
            echo "</tls-crypt>"
        } > /etc/openvpn/ccd/"$client".ovpn  #/etc/openvpn/ccd/"$client".ovpn
        else
...
new_pool() {
    ...
    if [ "$client_pool" != "1" ]; then
        ...
        echo "Create on Your requirement!"
    fi
    exit
}
...
new_hub_client() {
    ...
    if { [ "$client_pool_ip" != "1" ] || [ "$client_pool_ip" != "2" ]; } && [ "$client_pool_ip" != ' ' ]; then
        {
            #echo $(python3 /etc/openvpn/scripts/ip_process.py "$client")
            cat /etc/openvpn/server/client-common.txt
            echo "$client_pool_ip"
    ...
    elif [ "$client_pool_ip" == "1" ] || [ "$client_pool_ip" == ' ' ] ;then
...
verb 3" > /etc/openvpn/server/client-common.txt
	...
	echo "Finished!"
	echo
	echo "The client configuration is available in:" /etc/openvpn/ccd/"$client.ovpn"
	...
	case "$option" in
		...
		;;
			...
			echo "$VPN_HUB added. Configuration available in:" /etc/openvpn/hub_clients/"$VPN_HUB"
            		echo "New clients can be added by running this script again and choose on Your requirement."
			exit
		;;
        	6)
            		echo
